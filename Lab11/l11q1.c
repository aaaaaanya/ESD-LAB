...
void pwm_init(void);
void PWM1_IRQHandler(void);
unsigned long int i,j,x,col;
unsigned char flag,flag1;
int keyboard()
{
	int c;
	//p0.15 TO P0.18 FOR ROWS
	//p0.19 to p0.22 for columns
	
	
	LPC_GPIO0->FIOPIN=1<<15; //for row 0, p0.15
	 for(j=0;j<1000;j++);
		x=LPC_GPIO0->FIOPIN;
		x=(x>>19)&0xF;
		if(x==0)
...
void pwm_init(void)
{
	...
	LPC_PWM1->MR0 = 100; //setup match register 0 count
	LPC_PWM1->MR4 = 0x00000100; //setup match register MR1
	LPC_PWM1->LER = 0x000000FF; //enable shadow copy register
	LPC_PWM1->TCR = 0x00000002; //RESET COUNTER AND PRESCALER
	LPC_PWM1->TCR = 0x00000009; //enable PWM and counter
